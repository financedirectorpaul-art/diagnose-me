...
app.post('/ai/personal-check', (req, res) => {
  res.json({
    ...
    urgency: "Moderate",
    conditions: [
      { name: "Musculoskeletal issue", likelihood: "High", reason: "Symptoms" }
    ],
    overall_assessment: "Stable.",
    revenue_prompts: [{ message: "Add exam findings", value: 420 }],
...
app.post('/ai/diagnostics-assist', (req, res) => {
  ...
  res.json({
    image_assessment: description || "No description",
...
app.post('/ai/clinical-assist', (req, res) => {
  res.json({
    suggestions: ["Add laterality", "Add pain score"],
    funding_prompts: [
